...
public class WandEditorMenu extends AbstractContainerMenu {
    public final WandEditorEntity blockEntity;
    private final Level level;

    public WandEditorMenu(int containerId, Inventory inventory, FriendlyByteBuf extraData) {
        this(containerId, inventory, inventory.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public WandEditorMenu(int containerId, Inventory inventory, BlockEntity blockEntity) {
        super(ModMenuTypes.WAND_EDITOR_MENU.get(), containerId);
        this.blockEntity = ((WandEditorEntity) blockEntity);
        ...
        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 27, 8, 72) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(ModTags.Items.WAND_ITEM);
            }
        });

        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 28, 26, 72) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(ModTags.Items.KEY_ITEM);
            }
        });

        this.addSlot(new SlotItemHandler(this.blockEntity.inventory, 29, 44, 72) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return stack.is(ModTags.Items.KEY_ITEM);
            }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.WAND_EDITOR.get());
    }
